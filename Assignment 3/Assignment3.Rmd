...
df_pred <- df
colnames(df_pred)[which(names(df_pred) == 'p6b5')] <- 'depression'
df_pred['depression'] <- df_pred['depression'] - 1

```

Next we fit a logistic regression model to the data and summarize the results. As shown, the significant covariates are... The reason they are significant is because the values have a very low probability of being equal to zero (the null hypothesis), therefore the standard errors for the significant coefficients are relatively small compared to the magnitude of the coefficient's estimate.
...
plot(c(0,1), c(-1,1), xlab="Estimated  Pr (depression)", ylab="Observed - estimated", type="n", main="Residual plot", mgp=c(2,.5,0))
abline(0,0, col="gray", lwd=.5)
#points(pred_logistic, df_pred$depression-pred_logistic, pch=20, cex=.2)
...
The code below expresses the beta coefficients in terms of odds ratios
