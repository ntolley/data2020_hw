...
p <- ggplot(filtered_data_1960, aes(x=continent, y=population/10^6, fill=continent)) + geom_boxplot()  + theme_minimal() + labs(y="log population (in millions)", title="Continental population size in 1960") + scale_y_log10()
p
```
...
Repeating the same analysis as above, we see that only 3.9% of the dataset has an infant mortality above 150. Additionally we see that normal distribution is still not a good approximation for this data which exhibits a strong rightward skew, making the mean of the empirical data (and therefore the normal distribution) far from the mode empirical histogram.
